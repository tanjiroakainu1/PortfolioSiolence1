...
import { projectHost } from "../lib/projectDisplay";

export type ProjectCardItem = ProjectItem & { tag: string };

export function ProjectCardGrid({ items }: { items: ProjectCardItem[] }) {
  return (
    <ul className="project-cards__grid project-cards__grid--galaxy m-0 list-none p-0" role="list">
      {items.map((item) => (
        <li key={item.href} role="listitem" className="project-cards__wrap">
          ...
          >
            <span className="project-cards__thumb project-cards__thumb--placeholder" aria-hidden>
              <span className="project-cards__thumb-mark">{item.label.charAt(0)}</span>
