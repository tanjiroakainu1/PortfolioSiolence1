...
export function StackTechPanel({
  ...
  return (
    <div
      className={`rounded-2xl border border-neutral-400/22 bg-gradient-to-b from-[#141414]/95 to-[#0a0a0a]/98 p-3 shadow-[0_8px_32px_rgba(0,0,0,0.28),inset_0_1px_0_rgba(255,255,255,0.06)] ring-1 ring-white/[0.05] sm:p-4 ${className}`}
    >
      {eyebrow ? (
        <p className="m-0 mb-2.5 text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-neutral-300/80 sm:mb-3 sm:text-[0.65rem]">
          {eyebrow}
        </p>
      ) : null}
      <ul className="m-0 grid list-none gap-2 p-0 sm:gap-2" aria-label={listAriaLabel} role="list">
        {rows.map((row, i) => (
          <li
            ...
            role="listitem"
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-action/25 via-action-deep/15 to-netflix/12 text-action-bright shadow-[0_4px_14px_rgba(229, 229, 229,0.18)] ring-1 ring-action/35 sm:h-10 sm:w-10">
